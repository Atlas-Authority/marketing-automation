...
function mapCommonFields(info, partnerDomains) {
  const [firstName, ...lastNameGroup] = (info.name || ' ').split(' ');
  let lastName = lastNameGroup.filter(n => n).join(' ');

  const LASTNAME_URL_RE = /(.)\.([a-zA-Z]{2})/g;
  if (lastName.match(LASTNAME_URL_RE)) {
    lastName = lastName.replace(LASTNAME_URL_RE, '$1_$2');
  }
