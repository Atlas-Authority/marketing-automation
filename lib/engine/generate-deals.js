...
function interpretAsEvents(groups) {
  ...
  /** @type {EvalEvent | PurchaseEvent | null} */
  let event = null;
  for (const record of records) {
    if (
      ...
      event = { type: 'eval', license: record };
    }
    else if (
      ...
      if (isLicense(record)) {
        event = { type: 'purchase', license: record };
      }
      else {
        const license = (groups
          .map(g => g.license)
          .filter(l => l.addonLicenseId === record.addonLicenseId)[0]);
        event = { type: 'purchase', license, transaction: record };
      }
    }
  ...
  console.dir(events.map(e => e.type), { breakLength: 0 });
  console.log('\n\n\n');
