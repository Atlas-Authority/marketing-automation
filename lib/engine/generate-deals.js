      ...
      [
        ...licenses
          .map(l =>
            this.initialDeals
              .find(d =>
                l.addonLicenseId === d.properties.addonlicenseid)),
        ...groups.flatMap(g =>
          g.transactions
            .map(t =>
              this.initialDeals.find(d =>
                d.properties.transactionid === t.transactionId))),
      ].filter(isPresent),
      d => d.id);
 ...
 * @property {Transaction} refund
 * @property {Transaction} refunded
 */

/**
 * @typedef {RefundEvent} Event
 */
