...
class DealActionGenerator {
   ...
   */
  constructor(providerDomains, initialDeals) {
    this.providerDomains = providerDomains;
    this.initialDeals = initialDeals;
  }

  /**
   * @param {RelatedLicenseSet} relatedLicenseIds
   */
  generateActionsForMatchedGroup(relatedLicenseIds) {
    ...
    const badDomains = getBadDomains(licenses, this.providerDomains);
     ...
     * If Server/DC & Eval     & Eval Deal: Update Eval
     * If Server/DC & Purchase & No Deal:   Create Won, set Amount
     * If Server/DC & Purchase & Eval Deal: Close Won, set Amount, CloseDate
     * If Server/DC & Renewal/etc:          Create Won, set Amount
     * If Cloud & Eval & No Deal:           Create Eval
     * If Cloud & Purchase:                 Create or Update Closed Won, set CloseDate
     * If Cloud & Renewal/etc:              Create Won, set Amount
     * If Refund & Closed Deal:             Update CloseDate, Amount, Stage=Lost
     */
...
function generateDealActions(data) {
  const generator = new DealActionGenerator(data.providerDomains, data.initialDeals);
