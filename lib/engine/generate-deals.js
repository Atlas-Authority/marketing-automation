...
import { calculateLicenseTier } from './tiers.js';

/**
 * @param {License} license
 * @returns {Omit<Deal['properties'], 'dealstage'>}
 */
function dealPropertiesForLicense(license) {
  return {
    ...
    license_tier: calculateLicenseTier(license.evaluationOpportunitySize).toFixed(),
    ...
    dealname: `${license.addonName} at ${license.contactDetails.company}`,
...
export function generateDeals(data) {
  ...
  /** @type {Omit<Deal, 'id'>[]} */
  const dealsToCreate = dealCreateActions.map(({ license, dealstage, amount }) => {
    const contactIds = contactIdsForLicense(data.contactsByEmail, license);
    const properties = dealPropertiesForLicense(license);
  ...
  for (const { id, properties, license } of dealUpdateActions) {
    ...
    if (license) {
      ...
      const generatedProperties = dealPropertiesForLicense(license);
      newDeal = {
        id,
        properties: {
          ...generatedProperties,
 ...
 */
function olderThan90Days(dateString) {
  const now = Date.now();
