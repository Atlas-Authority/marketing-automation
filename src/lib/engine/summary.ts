...
export function printSummary(engine: Engine) {
  ...
  if (engine.dealManager.duplicates.size > 0) {
    ...
    engine.log?.warn('Deal Generator', 'Total of duplicates:', formatMoney(dupTotal));
    engine.log?.warn('Deal Generator', 'Total duplicates:', engine.dealManager.duplicates.size);
  ...
  engine.log?.info('Summary', 'Results of this run:');
  for (const row of table.eachRow()) {
    engine.log?.info('Summary', '  ' + row);
  }
