...
```sh
$ npm start -- --downloader=live --uploader=live
```
...
Instead of uploading to Hubspot, you can use `--uploader=console` and ENV variable `LOGLEVEL=verbose` (the default) to print data to console that would have been uploaded, or `LOGLEVEL=info` to just show array counts.
